...
void Main()
{
    // Each of the Result, Result<T> types have the same three Fail method overloads. The examples
    // below all use Result (because it's the simplest), but Result<T> both creates Fail results
    ...
    // When an exception is used to create the Fail result, the exception is mapped to the Fail
    // result's Error.
    Exception exception = GetException();
    Result.Fail(exception, errorCode: 500).Dump("Exception provided");
}
